...
namespace CloudBread.Controllers
{
    ...
    public class DWBaseCampResetController : ApiController
    {
        ...
        DWBaseCampResetDataModel GetResult(DWBaseCampResetDataInputParam p)
        {
            ...
            {
                string strQuery = string.Format("SELECT Gas, CashGas, Gem, CashGem, BaseCampList FROM DWMembersNew WHERE MemberID = '{0}'", p.memberID);
            ...
            if (DWMemberData.AddGas(ref gas, ref cashGas, totalMoney, 0, logMessage) == false)
            {
            ...
            using (SqlConnection connection = new SqlConnection(globalVal.DBConnectionString))
            {
                string strQuery = string.Format("UPDATE DWMembersNew SET Gas = @gas, CashGas = @cashGas, Gem = @gem, CashGem = @cashGem, BaseCampList = @baseCampList WHERE MemberID = '{0}'", p.memberID);
                using (SqlCommand command = new SqlCommand(strQuery, connection))
                {
