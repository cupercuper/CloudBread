...
namespace CloudBread.Controllers
{
    ...
    public class DWUdtStageInfoController : ApiController
    {
        ...
        DWUdtStageInfoModel GetResult(DWUdtStageInfoInputParams p)
        {
            ...
            {
                string strQuery = string.Format("SELECT LastWorld, CurWorld, CaptianChange, AllClear FROM DWMembersNew WHERE MemberID = '{0}'", p.memberID);
            ...
            using (SqlConnection connection = new SqlConnection(globalVal.DBConnectionString))
            {
                string strQuery = string.Format("UPDATE DWMembersNew SET LastWorld = @lastWorld, CurWorld = @curWorld, AllClear = @allClear WHERE MemberID = '{0}'", p.memberID);
                using (SqlCommand command = new SqlCommand(strQuery, connection))
                {
