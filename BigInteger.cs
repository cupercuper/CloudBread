...
namespace Keiwando.BigInteger
{
	...
	public class BigInteger
	{
        ...
        {
            int used = m_digits.DataUsed;
            writer.Write(used);
            for (int i = 0; i < used; ++i)
            {
                writer.Write(m_digits[i]);
            }
        }
        ...
        public void Read(BinaryReader reader)
        {
            ...
            DType[] array = new DType[used];
            for(int i = 0; i < used; ++i)
            {
                array[i] = reader.ReadUInt32();
            }

            m_digits = new DigitsArray(array);
        }

        private static string ToLongString(string input)//double input)
        {
